...
@onready var parent_menu : VBoxContainer = get_parent().get_node('ManorMenuContainer/Menu/VBoxContainer')
...
func _process(delta: float) -> void:
	if owner.manor_menu_open == true :
		...
		set_seed_selected(cursor_index)
	pass
...
func set_seed_selected(index: int) -> void: 
	get_parent().seed_selected = index;
	pass;
